...
public class ParkingListService implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (request.getParameter("key") != null) {
			...
			totcount = dao.parkingCount(s_query);
		} else {
			totcount = dao.parkingCount();
		}

		// 총 페이지수 계산
		int nowpage = 1; // 현재페이지
		int maxlist = 5; // 페이지당 글 수
		int totpage = 1; // 총 페이지
		...
		// 게시글 불러오기
		List<Parking_InfoVO> list = null;
		if (key.equals("")) {
			list = dao.parkingList(startpage, endpage);
		} else {
			list = dao.parkingList(s_query, startpage, endpage);
		}

		// 페이지 번호 생성
		String pageSkip = "";
		if (key.equals("")) {
			pageSkip = PageIndex.pageList(nowpage, totpage, "/Admin?cmd=parking_list&pages=park", addtag);
		} else {
			pageSkip = PageIndex.pageListHan(nowpage, totpage, "/Admin?cmd=parking_list&pages=park", query, key);
		}
		...
		RequestDispatcher rd = request.getRequestDispatcher("/Admin/parking_list.jsp");
