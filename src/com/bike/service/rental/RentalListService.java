package com.bike.service.rental;
...
public class RentalListService implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (request.getParameter("key") != null) {
			...
			totcount = dao.RentalCnt(s_query);
		} else {
			totcount = dao.RentalCnt();
		}
		...
		if (key.equals("")) {
			list = dao.RentalList(startpage, endpage);
		} else {
			list = dao.RentalList(s_query, startpage, endpage);
		}

		// 페이지 번호 생성
		String pageSkip = "";
		if (key.equals("")) {
			pageSkip = PageIndex.pageList(nowpage, totpage, "/Admin?cmd=rental_list&pages=rentalinfo", addtag);
		} else {
			pageSkip = PageIndex.pageListHan(nowpage, totpage, "/Admin?cmd=rental_list&pages=rentalinfo", query, key);
		}
		...
		RequestDispatcher rd = request.getRequestDispatcher("/Admin/rental_list.jsp");
